...
class Provider
{
	...
	{
		$sVPath = APP_INDEX_ROOT_PATH.'source/v/';
		...
		if (!empty($sTmp))
		{
			...
			if (\is_dir($sTmpFolder))
			{
				...
				if ($bResult && \file_exists($sTmpFolder.'/index.php') &&
					\is_writable(APP_INDEX_ROOT_PATH.'source/') &&
					\is_writable(APP_INDEX_ROOT_PATH.'index.php') &&
					\is_dir($sTmpFolder.'/source/'))
				{
					$aMatch = array();
					...
					else if (!\is_dir(APP_INDEX_ROOT_PATH.'source/v/'.$sNewVersion))
					{
						\MailSo\Base\Utils::CopyDir($sTmpFolder.'/source/', APP_INDEX_ROOT_PATH.'source/');

						if (\is_dir(APP_INDEX_ROOT_PATH.'source/v/'.$sNewVersion) &&
							\is_file(APP_INDEX_ROOT_PATH.'source/v/'.$sNewVersion.'/index.php'))
						{
						...
						else
						{
							if ($this->oLogger)
							{
								...
								$this->oLogger->Write($sTmpFolder.'/source/ -> '.APP_INDEX_ROOT_PATH.'source/', \MailSo\Log\Enumerations\Type::ERROR, 'INSTALLER');
							}
						}
					}
