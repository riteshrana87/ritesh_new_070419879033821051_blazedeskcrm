...
<div class="row">
    ...
    <!-- Search: Start -->
    <div class="col-xs-12 col-md-3 col-lg-3 col-sm-6 pull-right text-right ">
        <div class="row">
    ...
    <div class="col-xs-12 col-md-9 col-lg-9 col-sm-12">
        ...
        <?php } ?>
        <div class="row"> <div class="col-xs-4 col-md-4 col-sm-6 no-left-pad"><h3 class="white-link"><?= lang('opportunities') ?></h3></div>
            <div class="col-xs-8 col-md-8 col-sm-6 no-right-pad text-right">
                <?php //if (checkPermission('Opportunity', 'add')) { ?>
                            <!--<a data-href="<?php echo base_url('Opportunity/add'); ?>" data-toggle="ajaxModal" aria-hidden="true" data-refresh="true" class="btn btn-white add_opportunity" ><?= $this->lang->line('create_new_opportunity') ?></a>-->
                <?php //} ?>
                <?php if (checkPermission('Opportunity', 'add')) { ?>
                    <a data-href="<?php echo base_url($opportunity_view . '/importOpportunity'); ?>" aria-hidden="true" data-refresh="true" data-toggle="ajaxModal" title="<?= lang('import_opportunity') ?>" class="btn btn-white" ><?= $this->lang->line('import_opportunity') ?></a>
                <?php } ?>
                <?php if (checkPermission('Opportunity', 'add')) { ?>
                    <a title="<?= lang('EXPORT_OPPORTUNITY') ?>" class="btn btn-white" onclick="exportOpportunity();"><?= $this->lang->line('EXPORT_OPPORTUNITY') ?></a>
                <?php } ?>
            </div>
            <div class="clr"></div></div>
        <!-- Load ajax view table: Start -->
        <div  class="whitebox" id="common_div">
            <?php $this->load->view('AjaxOpportunityList'); ?>
        </div>
        <!-- Load ajax view table: End -->
    </div>
    <div class="col-xs-12 col-md-3 col-lg-3 col-sm-12">
        <div class=" whitebox">
            <div class="col-md-6 col-xs-6"> <span class="font-3em greencol"><b id="opportunity_count"><?php echo $total_opportunity; ?></b></span> </div>
            <div class="col-md-6 col-xs-6 no-left-pad">
                <p class="font-1em pad_top10"><b><?= lang('total') ?><br/>
                        <?= lang('opportunities') ?></b></p>
            </div>
            ...
            <div class="col-md-12 col-xs-12">
                ...
                <div class="form-group">
                    <select name="search_branch_id" class="form-control selectpicker chosen-select" id="search_branch_id" onchange="data_search();">
    ...
    <?php } ?>
                ...
                <div class="form-group">
                    <select name="search_prospect_owner_id" class="form-control chosen-select" id="search_prospect_owner_id" onchange="data_search();">
...
<?php if (isset($prospect_owner) && count($prospect_owner) > 0) { ?>
    <?php foreach ($prospect_owner as $prospect) { ?>
                                <option value="<?php echo $prospect['login_id']; ?>" <?php if (!empty($prospect_show_id) && $prospect_show_id == $prospect['login_id']) {
            echo "selected='selected'";
...
<div class="row">
<?php if (checkPermission('Opportunity', 'add')) { ?>
        <div class="col-md-2 col-new-71 col-xs-12"> <a class="btn btn-white full-width" data-href="<?php echo base_url('Opportunity/add'); ?>" data-toggle="ajaxModal" aria-hidden="true" data-refresh="true"><?php echo $this->lang->line('create_new_opportunity') ?></a> </div>
<?php } ?>
...
<?php if (checkPermission('Lead', 'add')) { ?>
       <!-- <div class="col-md-2 col-new-71 col-xs-12"> <a class="btn btn-white full-width" data-href="<?php echo base_url('Lead/add'); ?>" data-toggle="ajaxModal" aria-hidden="true" data-refresh="true"><?php echo $this->lang->line('create_new_lead') ?></a> </div>-->
<?php } ?>
    <!--<div class="col-md-2 col-new-71 col-xs-12"> <a class="btn btn-white full-width" href="<?php echo base_url('Lead'); ?>"><?php echo $this->lang->line('manage_leads') ?></a> </div>-->
<?php if (checkPermission('Account', 'add')) { ?>
        <div class="col-md-2 col-new-71 col-xs-12"> <a class="btn btn-white full-width" data-href="<?php echo base_url('Account/add'); ?>" data-toggle="ajaxModal" aria-hidden="true" data-refresh="true"><?php echo $this->lang->line('create_new_account') ?></a> </div>
<?php } ?>
    <div class="col-md-2 col-new-71 col-xs-12"> <a class="btn btn-white full-width" href="<?php echo base_url('Account'); ?>"><?php echo $this->lang->line('manage_account') ?></a> </div>
    ...
    <div class="col-md-2 col-new-71 col-xs-12"> <a href="#" class="btn btn-white full-width"><?php echo $this->lang->line('manage_reports') ?></a> </div>
</div>
<script>
...
<script type="text/javascript">
    ...
    function data_search(allflag)
    {
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#opportunity_count').text($('#total_opportunity_count').val());
            }
        });
        return false;
    }
    ...
    {
        var export_url = '<?php echo base_url() . "Opportunity/exportOpportunitys"; ?>';
        var delete_meg = "<?php echo lang('CONFIRM_EXPORT_OPPORTUNITY'); ?>";
        BootstrapDialog.show(
                {
                    title: '<?php echo $this->lang->line('Information'); ?>',
                    message: delete_meg,
