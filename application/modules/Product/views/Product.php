...
$this->load->view('Product/ajaxlist.php','',true);
?>
  ...
  <div class="clearfix visible-xs-block"></div>
    ...
    <div class="row">
      ...
      <div class="col-xs-12 col-sm-10 col-md-9  text-right">
        <?php if(checkPermission('Product','add')){?>
        <a data-href="<?php echo base_url($crnt_view.'/AddEditProduct');?>" data-toggle="ajaxModal" class="btn btn-white">
        <?= lang('create_product')?>
        </a>
        <?php } ?>
       
        <a href="<?php echo base_url().'ProductGroup';?>" class="btn btn-white" ><?= lang('product_group')?></a> 
        
         <?php if(checkPermission('Product','add')){?>
        <a data-href="<?php echo base_url($crnt_view.'/importProduct');?>" data-toggle="ajaxModal" class="btn btn-white" title="<?= lang('IMPORT_PRODUCT')?>"><?= lang('IMPORT_PRODUCT')?></a>
        <a onclick="exportProduct();" title=" <?= lang('EXPORT_PRODUCT')?>" class="btn btn-white"><?= lang('EXPORT_PRODUCT')?></a>
        <?php } ?>
...
<script>
    ...
    {
        var export_url = '<?php echo base_url()."Product/exportProduct";?>';
        var delete_meg ="<?php echo lang('CONFIRM_EXPORT_PRODUCT');?>";
