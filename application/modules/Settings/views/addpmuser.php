   ...
   <div class="modal-content">
              ...
              <div class="modal-body">
               	<div class="row"><div class="col-md-6 form-group">
                	<input type="text" data-parsley-pattern="^\d{0,9}?$" name="no_of_user" id="no_of_user" required class="form-control pm_user_new" placeholder="<?php echo lang('no_of_user');?>">
                	<input type="hidden" name="pm_amount" id="pm_amount" value=""/>
                </div>
                <div class="col-md-6 form-group">
                	<input type="text" name="current_user" id="current_user" class="form-control pm_user" value="<?php echo $setup[0]['pm_user'];?>" placeholder="<?php echo $setup[0]['pm_user'];?> <?php echo lang('current_user');?>" readonly>
                </div>
                <div class="col-md-6 form-group">
...
<div class="modal in" id="add_contact_data" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <?php
            $contact_path='Settings/checkout_pm';
...
//                Stripe.setPublishableKey('pk_test_suxHAAvKSymUCw8lxGk7ZxLs'); 
            ...
            $(document).ready(function() {
				...
				var pm_amount=20*$('.pm_user').val();
					
					   $('#current_user_price').text(pm_amount);
					   $('#current_user_price1').text(pm_amount);
					
					$('.pm_user_new').keyup(function(){
						 $pm_amount=20*$(this).val();
						$('#new_add_user').text($pm_amount);
					});
                 ...
                 var type=$('#type').val();
					...
					var users=$('#no_of_user').val();
					  if ($('#paymentfrm1').parsley().isValid()) {
							...
							$.ajax({
									type: 'POST',
									url: '<?php echo base_url("Settings/count_inactive_user?type=pm&setup_id="); ?>'+setup_id,
