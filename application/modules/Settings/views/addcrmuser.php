   ...
   <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><b>
					<?php if($project_view=='adddata'){
						echo lang('add_new_crm');
					}else{
						echo lang('remove_crm');
						}
						?>
					</b>
				</h4>
              ...
              <div class="modal-body">
               	<div class="row"><div class="col-md-6 form-group">
                	<input type="text" data-parsley-pattern="^\d{0,9}?$" name="no_of_user" id="no_of_user" required class="form-control crm_user_new" placeholder="<?php echo lang('no_of_user');?>">
                	<input type="hidden" name="crm_amount" id="crm_amount" value=""/>
                </div>
                <div class="col-md-6 form-group">
                	<input type="text" name="current_user" id="current_user" class="form-control crm_user" value="<?php echo $setup[0]['crm_user'];?>" placeholder="<?php echo $setup[0]['crm_user'];?> <?php echo lang('current_user');?>" readonly>
                </div>
                <div class="col-md-6 form-group">
...
<div class="modal in" id="add_contact_data" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <?php
            $contact_path='Settings/checkout_crm';
      ...
      <div class="modal-header">
        <button data-dismiss="modal" class="close" id="payment_close" type="button">×</button>
...
//                Stripe.setPublishableKey('pk_test_suxHAAvKSymUCw8lxGk7ZxLs'); 
			...
			$('#payment_close').click(function(){
					 ...
					 $("#add_contact_data").css("display", "none");
				
				});
					var crm_amount=20*$('.crm_user').val();

					   $('#current_user_price').text(crm_amount);
					   $('#current_user_price1').text(crm_amount);

					$('.crm_user_new').keyup(function(){

						 $crm_amount=20*$(this).val();
						$('#new_add_user').text($crm_amount);
					});
                 ...
                 var type=$('#type').val();
					...
					var users=$('#no_of_user').val();
					  ...
					  if ($('#paymentfrm1').parsley().isValid()) {
							...
							$.ajax({
									type: 'POST',
									url: '<?php echo base_url("Settings/count_inactive_user/?type=crm&setup_id="); ?>'+setup_id,
...
// disable the submit button to prevent repeated clicks
                if($('#card_number').val()==''){
                     $("#errorjs").html('<?= lang('enter_card_no');?>');
                    return false;
                }
                    ...
                    if($('.card-expiry-month').val()<currentMonth && $('.card-expiry-year').val()==currentYear){
                             $("#errorjs").html('<?= lang('invalidmonth');?>');
                            return false;
                    }
