...
<div id="main-page">
    <div class="row">
        <div class="col-xs-6 col-md-6 no-left-pad">
            <h3 class="white-link"><?= lang ('invoices') ?></h3>
        </div>
        <div class="col-xs-6 col-md-6 pull-right no-right-pad">
            <?php if (checkPermission ('Invoices', 'add')) { ?><a
                data-href="<?php echo base_url () . 'Projectmanagement/Invoices/add_record'; ?>"
                data-toggle="ajaxModal" title="<?= lang ('create') ?>"
                class="btn btn-white pull-right" ><?= lang('create') ?> <?= lang ('invoice') ?></a><?php } ?>
        </div>
    </div>
    
    <div class="whitebox" id="common_div">

        <?php $this->load->view ('InvoicesAjaxList') ?>
    </div>
    <div class="clr"></div>
