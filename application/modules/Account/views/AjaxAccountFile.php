...
<div class="whitebox" id="table_attach">
    ...
    if (isset($file_data) && count($file_data) > 0) {
        ...
        <?php if (checkPermission("Account", 'delete')) { ?><i class="fa fa-remove" title="<?= $this->lang->line('delete') ?>" onclick="delete_account_file('<?php echo $data['file_id']; ?>', '<?php echo $data['file_path'] ?>', '<?php echo $_SERVER['HTTP_REFERER']; ?>')"></i><?php } ?>
                </li>
    <?php
    }
} else {
...
<script>
    function delete_account_file(file_id, file_path, redirect_link)
    {
        var delete_url = "<?php echo base_url(); ?>Account/deleteAccountAttach/?file_id=" + file_id + "&file_path=" + file_path + "&redirect_link=" + redirect_link;
        var delete_meg = "<?php echo lang('CONFIRM_DELETE_FILE'); ?>";
