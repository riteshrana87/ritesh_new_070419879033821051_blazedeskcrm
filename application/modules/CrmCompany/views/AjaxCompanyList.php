...
<div class="table table-responsive">
    ...
    <table class="table table-striped dataTable" id="example1" role="grid" aria-describedby="example1_info" width="100%">
        <thead>
            <tr>
                <th <?php
                if (isset($sortfield) && $sortfield == 'company_name') {
                    if ($sortby == 'asc') {
                ...
                <th <?php
                if (isset($sortfield) && $sortfield == 'phone_no') {
                    if ($sortby == 'asc') {
                        echo "class = 'sorting_desc'";
                    } else {
                        echo "class = 'sorting_asc'";
                ...
                <th <?php
                if (isset($sortfield) && $sortfield == 'website') {
                    if ($sortby == 'asc') {
                        echo "class = 'sorting_desc'";
                    } else {
                        echo "class = 'sorting_asc'";
        ...
        <tbody id="postList">
                <?php if (isset($company_data) && count($company_data) > 0) { ?>
                    ...
                    <?php } else { ?>
                            <td><?php echo lang('inactive'); ?></td>
        <?php } ?>
                        <td class="bd-actbn-btn">
                    <?php if (checkPermission('CrmCompany', 'view')) { ?><a href="<?= base_url() ?>CrmCompany/view/<?php echo $data['company_id'] ?>"  title="<?= lang('view') ?>" ><i class="fa fa-search fa-x greencol"></i></a><?php } ?>
        <?php if (checkPermission('CrmCompany', 'edit')) { ?><a data-href="<?= base_url() ?>CrmCompany/edit/<?php echo $data['company_id'] ?>" aria-hidden="true" data-refresh="true" data-toggle="ajaxModal" title="<?= lang('edit') ?>" ><i class="fa fa-pencil fa-x bluecol"></i></a><?php } ?>
        <?php if (checkPermission("CrmCompany", 'delete')) { ?><a href="javascript:;" title="<?= $this->lang->line('delete') ?>" onclick="deleteItem('<?php echo $data['company_id']; ?>');"><i class="fa fa-remove redcol"></i></a><?php } ?>
                        </td>
                    </tr>
    <?php } ?>
        <?php } else { ?>
...
<script>
    ...
    {
        var delete_meg = "<?php echo lang('company_del_msg'); ?>";
        BootstrapDialog.show(
                {
                    ...
                    buttons: [{
                            ...
                            action: function (dialog) {
                                window.location.href = "<?php echo base_url('CrmCompany/deleteData/'); ?>/" + id;
                                dialog.close();
                            }
                        }]
                });
