...
?>
        ...
        <tr>
            ...
            <th class="col-md-1"> <?php echo lang('actions');?>
            ...
            <?php foreach($contact_info as $data){ ?>
                <tr>
                    ...
                    <td class="col-md-1 bd-actbn-btn">
                        <?php if(checkPermission('Contact','view')){ ?><a href="<?=base_url('Contact/view/'.$data['contact_id'])?>" class="edit_contact" title="<?php echo lang('view');?>" ><i class="fa fa-search fa-x greencol"></i></a><?php } ?>
 
                        <?php if(checkPermission('Contact','edit')){ ?><a data-href="<?=base_url()?>Contact/edit/<?=$data['contact_id']?>" aria-hidden="true" data-refresh="true" data-toggle="ajaxModal" title="<?=lang('edit')?>" class=""><i class="fa fa-pencil fa-x bluecol"></i></a><?php } ?>

                        
                        <?php if (checkPermission('Contact', 'delete')) { ?><a data-href="javascript:;" title="<?php echo lang('delete');?>" onclick="delete_request('<?php echo $data['contact_id']; ?>','<?php echo base_url('Contact');?>');" ><i class="fa fa-remove fa-x redcol"></i></a><?php } ?>
                    </td>
                </tr>
            <?php }?>
      <?php }else{?>
...
{
    var delete_url = "Contact/deletedata/?id=" + contact_id+"&link="+redirect_link;
