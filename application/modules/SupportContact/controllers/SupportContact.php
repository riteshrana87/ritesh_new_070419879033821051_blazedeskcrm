...
class SupportContact extends CI_Controller {
    ...
    public function index() 
    {
       ...
       $this->breadcrumbs->push(lang('support'), '/Support');
        ...
        if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
            $this->session->unset_userdata('supportcontact_data');
        }

        $searchsort_session = $this->session->userdata('supportcontact_data');
...
//Get Total Records From CONTACT_MASTER Table
        $table_total_contact = CONTACT_MASTER . ' as cm';
       
        $fields_total_contact = array("count(cm.contact_id) as total_contact");
        $total_contact = $this->common_model->get_records($table_total_contact, $fields_total_contact, '', '', $where);
        $data['total_contact'] = $total_contact[0]['total_contact'];
// If have any search than give post data in db
        ...
        $this->session->set_userdata('supportcontact_data', $sortsearchpage_data);
        $data['drag']=true;
    ...
    public function addrecord() 
    {
       if (!$this->input->is_ajax_request()) {
        ...
        $data['checkUserIsCrm'] = checkUserIsCrm($this->user_info['ID']);
...
//            $this->session->set_flashdata('msg', $msg);
        ...
        $match22 = "bm.branch_name='" . $branchName . "' and status=1 ";
...
//upload image
       ...
       if($email_client_success_insert){
        $success_insert = $this->common_model->insert(CONTACT_MASTER, $contactdata);
            ...
            if ($success_insert) {
                ...
                $checkUserIsCrm = checkUserIsCrm($this->user_info['ID']);
                // start Added by Sanket For adding contact in newsleeter
                if($is_newsletter == "on" && $checkUserIsCrm)
                {
    ...
    public function updatedata() 
    {
        ...
        {
            $redirect_link = base_url()."SupportContact/view/".$id;
...
//Update Record in Database
    ...
    public function deletedata() {
        ...
        if (!empty($id)) {
            ...
            if ($success_delete) 
            {
                ...
                $newzleter_type = get_newsletter_type();
                $checkUserIsCrm = checkUserIsCrm($this->user_info['ID']);
                if($newzleter_type == '1' && $checkUserIsCrm)
                {
    ...
    public function view($id = NULL)
    {
        ...
        {
           $this->breadcrumbs->push(lang('support'), '/Support');
           $this->breadcrumbs->push(lang('contacts'), '/SupportContact');
            ...
            $params['join_tables'] = array(TBL_CRM_JOB_TITLE . ' as jt' => 'jt.job_title_id=cm.job_title',COMPANY_MASTER . ' as co' => 'co.company_id=cm.company_id');
            $params['join_type'] = 'left';
            //Get Records From CONTACT_MASTER Table       
            $table5 = CONTACT_MASTER . ' as cm,'.COUNTRIES.' as c';
            $match5 = "co.country_id = c.country_id AND cm.is_delete= 0 AND cm.contact_id = " . $id;
    ...
    function importContactdata()
    {
        ...
        $config['upload_path'] = './uploads/csv_contact';
        ...
        else
        {
            ...
            foreach ($cell_collection as $cell)
            {
                ...
                if(trim(strtolower($value_language)) == 'english')
                {
                    $language_id = 1;
                }else
                {
                ...
                $data['address1'] = $value_address_1;
                $data['address2'] = $value_address_2;
    ...
    function insertContactToCampaign()
    {
        ...
        //$redirect_implode=  explode('_', $redirect_explode);
        //$redirect_link=  implode('/', $redirect_implode);
        if (strpos($redirect_link, 'Lead/viewdata') !== false) {
    ...
    public function inserttask() 
    {
        ...
        //$taskData['before_status'] = $this->input->post('before_after');
        //$taskData['repeat'] = $this->input->post('repeat');
        //$taskData['time'] = date('h:i:s a', strtotime($this->input->post('remind_time')));

        //$taskData['time'] = $this->input->post('remind_time');
        //$taskData['remind_before_min'] = $this->input->post('remind_day');
...
//        $data['note_data'] = $this->common_model->get_records(TBL_NOTE . ' as td', $fields, '', '', '', '', $config['per_page'], $data['page'], $data['contatcSortField'], $data['contactSortOrder'], '', $where,'','','','','',$where_search);
    ...
    function viewNoteData()
    {
        ...
        {
            $searchtext1 = $this->session->userdata('searchtext');
            $where_search = '(td.note_subject LIKE "%' . $searchtext1 . '%" OR td.note_description LIKE "%' . $searchtext . '%")';
        }else
        {
            $this->session->set_userdata('searchtext','');
        }
    ...
    public function viewTaskData()
    {
        ...
        elseif(strpos($redirect_link, 'SupportContact') !== false) {
            $task_status = 1;
        }
        ...
        $data['tasksortOrder'] = 'desc';
        $fields = array("td.task_id,td.task_assign,td.task_related_id,td.task_name,td.importance,td.remember,td.task_description,td.start_date,td.end_date,td.status,td.is_delete,td.created_date");
    ...
    function viewAllDealsData()
    {
        ...
        /*$fields = array("pm.*"); */
        $fields = array("pm.prospect_id,pm.prospect_assign,pm.prospect_related_id,pm.prospect_name,pm.prospect_auto_id, pm.status,pm.status_type,pm.creation_date,count(orc.prospect_id) as contact_count,cm.contact_name");
        $where = array("pm.is_delete" => "0","pm.status" =>"1","pm.status_type" => "1","pm.company_id"=>$contact_id);
        $group_by = 'pm.prospect_id';
        $config['total_rows'] = count($this->common_model->get_records(PROSPECT_MASTER . ' as pm', $fields, $params['join_tables'],$params['join_type'], '', '', '', '', $data['tasksortField'], $data['tasksortOrder'],$group_by, $where,'','','','','',$where_search));
...
//
    ...
    function insertEvent()
    {
        ...
        $data['event_date'] = date('Y-m-d H:i',  strtotime($this->input->post('event_date')));
...
//
    ...
    public function viewCasesRecord($id='') {
        ...
        if (!empty($id)) {
            ...
            $data['modal_title'] = 'View Project Cases';
...
//            $match = "et.template_id =".TEMPLATE_SCHEDULE_MEETING;
...
//            $template = $this->common_model->get_records($table, $fields, '', '', $match);
    ...
    public function getContactMeeting()
    {
        ...
        if (!empty($searchtext)) 
        {
           ...
           $where_search = '(td.meet_title LIKE "%' . $searchtext . '%" )';
           $this->session->set_userdata('searchtext', $searchtext);
        }else if(!empty($session_searchtext))
        {
            $searchtext1 = $this->session->userdata('searchtext');
            $where_search = '(td.meet_title LIKE "%' . $searchtext1 . '%" )';
        }else
        {
            $this->session->set_userdata('searchtext','');
        }
        ...
        //config variable for the pagination
        $config['base_url'] = base_url('SupportContact') . '/getContactMeeting';
...
//        }
  ...
  function addTask($contact_id = null) {
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        } else {
        ...
        $data['main_content'] = '/SupportContact';
    ...
    public function edittask($id) {
        ...
        $match = "tm.task_id = " . $id;
        $fields = array("tm.task_id,tm.task_related_id,tm.task_assign,tm.task_name,tm.importance,tm.remember,tm.task_description,tm.start_date,tm.end_date");
...
//Get Records From PROSPECT_MASTER Table with JOIN
            $data = array();
            $params['join_tables'] = array(
                COUNTRIES . ' as c' => 'c.country_id=cnm.country_id',
               );
            $params['join_type'] = 'left';
            $match = "cnm.contact_id = " . $id;
            $table = CONTACT_MASTER . ' as cnm';
            $groupBy = 'cnm.contact_id';
            $fields = array("cnm.contact_id,cnm.company_id,"
                . "cnm.address1,cnm.address2,cnm.city,cnm.state,c.country_name");
