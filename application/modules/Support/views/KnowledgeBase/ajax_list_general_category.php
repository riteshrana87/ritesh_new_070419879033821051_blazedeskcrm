                ...
                <?php foreach ($sub_category_info as $data) { ?>
                    ...
                    <td>
                        ...
                        <?php $redirect_link = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REDIRECT_URL'];?>
                         <?php if(checkPermission("KnowledgeBase",'delete')){ ?><a href="javascript:;" title="<?= $this->lang->line('delete') ?>" onclick="deleteItem('<?php echo $data['sub_category_id']; ?>','<?php echo $data['main_category_id']; ?>');" title="<?= lang('delete') ?>"><i class="fa fa-remove redcol"></i></a><?php } ?>
                    </td>
                    </tr>
                <?php } ?>
            <?php } else { ?>
...
function deleteItem(id,mid)
    {
        var delete_meg ="<?php echo lang('sub_category_delete_message'); ?>";
        BootstrapDialog.show(
            {
                ...
                buttons: [{
                    ...
                    action: function(dialog) {
                        window.location.href = "<?php echo base_url('Support/KnowledgeBase/deletesub/'); ?>/" + id + "/" + mid;
                        dialog.close();
                    }
                }]
            });
