...
//$formAction = !empty($edit_record)?'updatedata':'insertdata'; 
$TaskAction = 'inserttask';
$taskPath = 'Task/' . $TaskAction;
?>
            ...
            <div class = " row">
                <div class="row">  <div class = "col-xs-12 col-sm-12 form-group">
                        ...
                        if (!empty($edit_record[0]['task_name'])) {
                            echo htmlentities(stripslashes($edit_record[0]['task_name']));
                        }
                        ?>">
                    </div></div>
                    ...
                    <div class ="col-xs-12 col-sm-4 form-group">
                        ...
                        <div class="input-group date" id="end_date">
                            ...
                            if (!empty($edit_record[0]['end_date']) && $edit_record[0]['end_date'] != '0000-00-00') {
                                echo configDateTime($edit_record[0]['end_date']);
                            }
                            ?>">
                            <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span> </div>
                        <div id="end-date"></div>
                ...
                <div class ="row">
                    <div class = "col-xs-12 col-sm-3">
                        ...
                        <div class="">
                            ...
                            <input <?php if (!empty($edit_record[0]['remember'])) { ?>checked="checked"<?php } ?> data-toggle="toggle" data-on="<?php echo lang('on'); ?>" data-off="<?php echo lang('off'); ?>" data-onstyle="success" type="checkbox"  id="reminder" name="reminder" onChange="toggle_show(<?php echo "'#" . $reminder_id . "'"; ?>, this)"/>

                        </div>
                    </div>
                    <div id="<?php echo $reminder_id; ?>" class="col-sm-9 col-xs-12">
                        ...
                        <div class = "col-xs-12 col-sm-6 bd-form-group">
                            <div class="row"><div class="col-sm-5"><label><?= $this->lang->line('REMINDER_DATE') ?> : *</label></div>
                                <div class="input-group date col-sm-7" id="reminder_date">
                                    ...
                                    <input type="text" class="form-control reminder_date" placeholder="<?= $this->lang->line('REMINDER_DATE') ?>" id="reminder_date" data-parsley-errors-container="#remind-date" name="reminder_date" onkeydown="return false" value="<?php
                                    if (!empty($edit_record[0]['reminder_date']) && $edit_record[0]['reminder_date'] != '0000-00-00') {
                                        echo configDateTime($edit_record[0]['reminder_date']);
                                    }
                                    ?>" >
                                    <span class="input-group-addon"> <span class="glyphicon glyphicon-calendar"></span> </span> </div>
                        ...
                        <div class = "col-xs-12 col-sm-6 bd-form-group">
                            <div class="row"> <div class="col-sm-5"><label><?= $this->lang->line('REMINDER_TIME') ?> : *</label></div>
                                <div class="input-group date col-sm-7" id="reminder_time">
                                    ...
                                    <input type="text" class=" form-control reminder_time" placeholder="<?= $this->lang->line('REMINDER_TIME') ?>" id="reminder_time" name="reminder_time" onkeydown="return false" value="<?php
                                    if (!empty($edit_record[0]['reminder_time']) && $edit_record[0]['reminder_time'] != '00:00:00') {
        ...
        <div class="modal-footer">
            <div class="text-right">
                ...
                <?php if (isset($edit_record)) { ?>
                    <?PHP if (checkPermission('Task', 'delete')) { ?> <a data-href="javascript:;" onclick="delete_request('<?php echo $edit_record[0]['task_id']; ?>', '<?php echo $redirect_link; ?>');" ><input type="button" class="btn btn-primary" name="remove"  value="<?= $remove_button_title ?>" /></a><?php } ?>
                    <?PHP if (checkPermission('Task', 'edit')) { ?> <a data-href="javascript:;" onclick="complete_request('<?php echo $edit_record[0]['task_id']; ?>', '<?php echo $redirect_link; ?>');" ><input type="button" class="btn btn-primary" name="completed"  value="<?= $this->lang->line('complete') ?>" /></a><?php } ?>
                    <?php } ?>
            </div>
        </div>
    </div>
...
<script>
    ...
    function delete_request(task_id, redirect_link) {
        task_id = $('#task_id').val();
        var delete_url = "<?php echo base_url(); ?>Task/deletetask/?id=" + task_id + "&link=" + redirect_link;
    ...
    function complete_request(task_id, redirect_link) {
        task_id = $('#task_id').val();
        var complete_url = "<?php echo base_url(); ?>Task/completeTask/?id=" + task_id + "&link=" + redirect_link;
    ...
    $(document).ready(function () {
        ...
        window.Parsley.addValidator('gteq',
                ...
                .addMessage('en', 'le', '<?php echo lang('less_equal_value'); ?>');
...
<?php } ?>
    ...
    $('#end_date')
            .datepicker({autoclose: true, startDate: '-0m'
            }).on('changeDate', function (selected) {
...
<?php } ?>
    ...
    //if toggle checked  required field
...
if (!empty($id)) {
    ?>
        $('#reminder_date').datepicker({format: 'MM-DD-YYYY'});
<?php } else {
    ?>
        $('#reminder_date').datepicker({minDate: current_date, format: 'MM-DD-YYYY'});
<?php } ?>
