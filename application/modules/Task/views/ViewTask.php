...
<div class="modal-dialog">
    ...
    <div class="modal-content">
        ...
        <div class="modal-header">
            ...
            <h4 class="modal-title"><div class="modelTaskTitle"><?= $this->lang->line('task_details') ?>:<?php if (!empty($editRecord[0]['status']) && ($editRecord[0]['status']) == 2) { ?>
                ...
                <div class = "col-sm-4 col-xs-12">
                    ...
                    <div class="input-group date" id="start_date">
                        ...
                        if (!empty($editRecord[0]['start_date']) && $editRecord[0]['start_date'] != '0000-00-00') {
                            echo configDateTime($editRecord[0]['start_date']);
                        }
                    ?></p>
                    </div>
                    ...
                    <div class="input-group date" id="end_date">
                        <p><?php
                            if (!empty($editRecord[0]['end_date']) && $editRecord[0]['end_date'] != '0000-00-00') {
                                echo configDateTime($editRecord[0]['end_date']);
                            }
                    ?></p>
                    </div>
                </div>
...
<?php } else { ?>
    ...
    if (!empty($editRecord[0]['reminder_date']) && $editRecord[0]['reminder_date'] != '0000-00-00') {
        echo configDateTime($editRecord[0]['reminder_date']);
    }
    ?></p>
                        </div>
...
<?php $redirect_link = $_SERVER['HTTP_REFERER']; ?>
            <?php if ((!empty($editRecord[0]['status'])) && ($editRecord[0]['status'] == 2)) { ?>
                <?PHP if (checkPermission('Task', 'edit')) { ?> <a data-href="javascript:;" onclick="reopen_request('<?php echo $editRecord[0]['task_id']; ?>', '<?php echo $redirect_link; ?>');" ><input type="button" class="btn btn-primary" name="completed"  value="<?= $this->lang->line('reopen') ?>" /></a><?php } ?>
            <?php } ?>
        </div>
    </div>
...
<script>
    ...
    function reopen_request(task_id, redirect_link) {
        ...
        var complete_url = "<?php echo base_url(); ?>Task/reopen/?id=" + task_id + "&link=" + redirect_link;
