...
$array_list = array("2","3","29","36","37","38","39","40","45","51");    
        ...
        <table id="emailTemplateTable" class="table table-striped">
            ...
            <tr>
                <th class='sortEmailTemplateList'>
                            <a  href="<?php echo base_url(); ?>Emailtemplate/index/<?= $page ?>/?orderField=subject&sortOrder=<?= $sortOrder ?>" >
                            <?php
					...
					<!-- 	<th><?= lang('emailTemplate_body')?></th>  -->
                        <th class='sortEmailTemplateList'>
                            <a  href="<?php echo base_url(); ?>Emailtemplate/index/<?= $page ?>/?orderField=status&sortOrder=<?= $sortOrder ?>">
                            <?php
            ...
            <tbody>
		        ...
		        <tr>		       
		          ...
		          <td><?php if(checkPermission('Emailtemplate','view')){ ?><a data-href="<?php echo base_url($crnt_view.'/viewEmailTemplate/'.$data['template_id']);?>" data-toggle="ajaxModal" title="<?=$this->lang->line('view')?>"><i class="fa fa-search fa-x greencol" ></i></a><?php }?>&nbsp;&nbsp;<?php if(checkPermission('Emailtemplate','edit')){ ?><a data-href="<?php echo base_url($crnt_view.'/edit/'.$data['template_id']);?>" data-toggle="ajaxModal" title="<?=$this->lang->line('edit')?>"><i class="fa fa-pencil bluecol" ></i></a><?php }?>&nbsp;&nbsp;<?php if(checkPermission("Emailtemplate","delete")){ if($data['system_template'] == 0) {?><a data-href="javascript:;" onclick="delete_request(<?php echo $data['template_id']; ?>);" title="<?=$this->lang->line('delete')?>"><i class="fa fa-remove redcol" ></i></a><?php } } ?></td>
		        </tr>
		        <?php } ?>
		      <?php } else { ?>
                <tr>
